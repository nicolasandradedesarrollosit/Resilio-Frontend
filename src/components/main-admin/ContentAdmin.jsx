...
import { USERS_PER_PAGE, INITIAL_USER_FORM, MESSAGES } from '../../helpers/constants';
...
function ContentMain() {
    ...
    const [editFormData, setEditFormData] = useState(INITIAL_USER_FORM);
    ...
    const loadUsers = async () => {
        try {
            ...
            const data = await getUsers(USERS_PER_PAGE, offset);
            
            if (data.ok) {
                setUsers(data.data);
            } else {
                throw new Error(data.message || MESSAGES.ERROR_LOADING_USERS);
            }
        } catch (err) {
    ...
    const handleNextPage = () => {
        if (canGoNext(filteredUsers.length, USERS_PER_PAGE)) {
            setCurrentPage(currentPage + 1);
        }
    };

    const { start: beginUsers, end: beginUsersFinal } = calculatePageRange(
        currentPage,
        USERS_PER_PAGE,
            ...
            <div className='admin-users-table-container'>
                ...
                <div className='admin-users-pagination'>
                    <div className='admin-users-info'>
                        <span>Usuarios por página: </span><span>{USERS_PER_PAGE}</span>
                    </div>
                    <div className='admin-users-page-buttons'>
                        ...
                        <button 
                            className={`admin-users-btn-page ${!canGoNext(filteredUsers.length, USERS_PER_PAGE) ? 'admin-users-disabled' : ''}`}
                            onClick={handleNextPage}
                            disabled={!canGoNext(filteredUsers.length, USERS_PER_PAGE)}
                        >
                    ...
                    <div className='admin-users-modal-content admin-users-modal-delete' onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className='admin-users-modal-body'>
                            ...
                            <p className='admin-users-delete-message'>
                                {MESSAGES.CONFIRM_BAN} <strong>{selectedUser?.name}</strong>?
                            </p>
                            <p className='admin-users-delete-warning'>
                                {MESSAGES.BAN_WARNING}
                            </p>
                        </div>
                        <div className='admin-users-modal-actions'>
                            <button 
                    ...
                    <div className='admin-users-modal-content admin-users-modal-unban' onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className='admin-users-modal-body'>
                            ...
                            <p className='admin-users-unban-message'>
                                {MESSAGES.CONFIRM_UNBAN} <strong>{selectedUser?.name}</strong>?
                            </p>
